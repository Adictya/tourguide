...
- Topic sidebar with every code section as a navigable item.
- Source files opened in real buffers without modifying contents.
- Code sections focused one at a time by dimming surrounding lines with extmarks and virtual notes.
...
On first tour start, TourGuide prompts for temporary normal-mode mappings for next section, previous section, next topic, and previous topic. Next/previous section moves through every section in the tour, including multiple sections from the same file. Choices are saved to `stdpath("state")/tourguide/keymaps.json`; submitting an empty answer disables that mapping.
