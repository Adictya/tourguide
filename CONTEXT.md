...
ELIC (Explain Like I Code) defines the language for task-scoped Explanations that can be generated by agents and presented across local Presentation Surfaces.

ELIC is presentation-first: it turns a tailored AI answer about code into a navigable, grounded Explanation Session. Common questions include why a bug happened, how a PR works, how a code path executes, or how a library architecture is changing.

The Explanation artifact is the transport and validation boundary between agents and Presentation Surfaces. An Explanation may be disposable and opened once, or saved for later discovery and revision. ELIC should not be framed as a wiki, docs site, living documentation platform, or durable onboarding tour system.

## Language

**Explanation**:
A portable artifact that organizes Topics, ordered Steps, and grounded Anchors for guided code understanding. An Explanation is the authored object, not the viewer session, rendered screen, act of walking through code, or decision to save it for later.
_Avoid_: Tour, walkthrough, explanation session, rendered explanation

**Explanation Goal**:
The author's reason for later revising or refreshing a saved Explanation, such as architecture overview, code path explanation, PR explanation, implementation summary, or debug diagnosis. An Explanation Goal is primarily LLM-facing maintenance intent; it does not define a different kind of Explanation, restrict which Steps, Flows, or Anchors may appear, or need to exist for one-off Explanations.
_Avoid_: Explanation type, explanation kind, artifact subtype

**Explanation Description**:
A concise user-facing summary of what an Explanation contains. Explanation Description helps users choose an Explanation when a Presentation Surface lists recent or saved Explanations, while Explanation Goal captures durable revision intent when needed.
...
Dev: "Is ELIC a wiki or living docs system?"

Domain expert: "No. ELIC is for presenting tailored AI answers about code. Some Explanations may be saved, but maintaining a project knowledge base is not the core use case."

Dev: "Do most Explanations need to be committed to the repo?"

Domain expert: "No. Many Explanations are one-off answers for debugging, PR review, or architecture inspection. Save one only when later discovery or revision is useful."

Dev: "If the artifact is disposable, why have an artifact at all?"

Domain expert: "Because the artifact is the transport and validation boundary between the agent and the Presentation Surface. It lets a one-off answer become navigable, grounded, and inspectable."
...
Domain expert: "No. The Explanation Goal preserves why a saved Explanation should be refreshed; it does not create a separate Explanation type or constrain every Step."

Dev: "Should the Explanation picker show the Explanation Goal?"

Domain expert: "Usually no. The picker should show the Explanation Description because it summarizes what the user will see. The Explanation Goal preserves durable revision intent."

Dev: "Does every one-off Explanation need an Explanation Goal?"

Domain expert: "No. Explanation Goal is useful when future revision needs the original intent; it is not required just to present the Explanation."
