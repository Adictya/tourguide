# Adopt the v1 Explanation contract

Status: accepted

ELIC's scaffold schema allowed Step titles, multiple Anchors per Step, Step-level presentation kinds, Anchor notes, and legacy viewer assumptions. We decided to replace that scaffold with the v1 Explanation contract: titleless Steps with one optional Anchor, first-class Flows for adjacent multi-Step context, additive Detail Levels, and generated hydration metadata for reliable evidence presentation.
